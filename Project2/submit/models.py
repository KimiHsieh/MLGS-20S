...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int, alpha: float, batch_size: int) -> Tuple[int,
        ...
        # Sampling from f() with inputs(x+noiz), these samples are used for "selection" by MC
        # n0 := the number of MC samples used for "selection"
        selcs_in_f_Xnoiz = self._sample_noise_predictions(inputs, n0, batch_size)
        
        # Find the top class in these "selections"(or samples) as guesses
        # ex. top_class = (k1,k3,k5,....)
        top_class = selcs_in_f_Xnoiz.argmax().item()
        
        # Sampling from f() with inputs(x+noiz), these samples are used for "estimation" by MC
        # num_samples := the number of MC samples used for "estimation"
        ests_in_f_Xnoiz = self._sample_noise_predictions(inputs, num_samples, batch_size)
        
        # Find a lower bound on pA by "estimation samples with guess"
        # !!! Compute the lower bound on the probability of the event occuring in a Bernoulli distribution. !!!
        num_class_A = ests_in_f_Xnoiz[top_class].item()
    ...
    def predict(self, inputs: torch.tensor, num_samples: int, alpha: float, batch_size: int) -> int:
        ...
        # !!! See docstring Plz!!!
        # class_counts: = Sampling from f() with inputs(x+noiz), these samples are used for "estimation" by MC
        # num_samples := the number of MC samples used for "estimation"
        num_class = class_counts.numpy().shape[0]
        top2_classes = class_counts.argsort()[(num_class-2):num_class] # Find numbers of top2 classes from class_counts 
        count1 = class_counts[top2_classes[0]] #Top2
        count2 = class_counts[top2_classes[1]] # Top1
        if binom_test(count2, count1 + count2, p=0.5) > alpha:
            return SmoothClassifier.ABSTAIN
        else:
            return top2_classes[1]
    ...
    def _sample_noise_predictions(self, inputs: torch.tensor, num_samples: int, batch_size: int) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            for it in range(ceil(num_samples / batch_size)):
                ...
                # batch.shape = (this_batch_size, C, N, N) See. pytorch repeat()
                batch = inputs.repeat((this_batch_size, 1, 1, 1))
                #noiz = torch.randn_like(batch, device='cuda') * self.sigma # create noise
                noiz = torch.randn_like(batch)*self.sigma
                x_pert = torch.clamp(batch+noiz, 0, 1)
                preds = self.base_classifier(x_pert).argmax(1) # Find the max prob after f(x_pert)
