...
def fast_gradient_attack(logits: torch.Tensor, x: torch.Tensor, y: torch.Tensor, epsilon: float, norm: str = "2",
    ...
    if (norm == "2"):
        loss_grad_norm = torch.norm(loss_grad,2, dim = (1,2,3)).view(-1,1,1,1)
        x_pert = x + epsilon * loss_grad/loss_grad_norm

    elif (norm == "inf"):
        x_pert = x + epsilon * torch.sign(loss_grad)

    else:
        loss_grad_norm = torch.norm(x, 1)
        x_pert = x + epsilon * loss_grad_norm
