...
def predict_model(model: nn.Module, dataset: Dataset, batch_size: int, attack_function: Union[Callable, None] = None,
    ...
    for x, y in tqdm(iter(test_loader), total=num_batches):
        ...
        if attack_function is None:
            ...
            y_pert = model(x_pert)
            pred = y_pert.argmax(dim=1, keepdim=True).view(-1)
            predictions.append(pred)
            targets.append(y)
...
def evaluate_robustness_smoothing(base_classifier: nn.Module, sigma: float, dataset: Dataset,
    ...
    for x, y in tqdm(iter(test_loader), total=len(dataset)):
        ##########################################################
        # YOUR CODE HERE
        #logits = base_classifier(x).argmax()
        y_predict = model.predict(x,num_samples=num_samples_2,alpha=alpha,batch_size=certification_batch_size)
        top_class, radius = model.certify(x, num_samples_1, num_samples_2, alpha, certification_batch_size)
        
        if( y_predict == SmoothClassifier.ABSTAIN ):
            abstains+=1
            radii.append(0)
            continue
            
        elif( top_class == SmoothClassifier.ABSTAIN ):
            abstains+=1
            radii.append(0)
            continue
